...
- **Never-weaken-a-trust-check**: reject any diff that deletes/relaxes/skips a self-test, a security-label route, the arm-side `trust_surface_paths_touched` classifier, or a `match_labels` keyword set. Decision 7 REVISED (maintainer 2026-07-18): an APPROVED trust-surface change AUTO-ARMS with a SHA-bound post-merge audit trail (`trust-surface` label + touched-path comment) — auto-arming on approval is the DESIGNED behavior, not a blocker; what remains blocking is any diff that weakens the audit trail, the head/base binding, or the injection/tamper human-stops.
