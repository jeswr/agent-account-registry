...
- **Retry via `scripts/gh_retry.py` for idempotent reads; NEVER wrap a CAS/ledger write or a mutation-confirmation in `run_gh`.** Transient 5xx/secondary-403/connection blips on a read get the shared bounded backoff; a replayed mutation can double-dispatch a worker (#559) and a `run_gh`-wrapped ledger CAS write both consumes the conflict signal its caller's re-read loop keys on and replays a stale expected-SHA (#558). A ledger writer that must survive a throttle takes the CLASSIFIER + WAIT SCHEDULE from `scripts/ledger_retry.py` and retries in its OWN loop, re-reading and re-deriving the expected SHA every time (`select-and-claim.py` is the reference). Do not hand-roll new retry/sleep loops.
...
CI-lane delta: your primary surface is .github/workflows/ — actionlint + PyYAML parse are HARD gates on every touched workflow; SHA-pin all actions; never widen job permissions without stating why in the PR body; a workflow change that touches token minting or trust checks is a trust-surface change (expect the human arm gate).
