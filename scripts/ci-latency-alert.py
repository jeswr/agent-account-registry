...
# --- M2 -----------------------------------------------------------------------------
# MEASURED run-level queue wait over COMPLETED runs on this repo: N=3,920, p50/p90/p99 all
# 0s, max 99.5m. ZERO-INFLATED — only 8 of 3,920 (0.20%) ever recorded a non-zero wait.
# That is why this is NOT stated as a multiple of the observed maximum: multiplying a max
# of 99.5m gives a threshold nothing reaches, and multiplying a p99 of 0s gives 0. For this
# shape the honest statement is the false-positive rate. 15 min fires on 6 of 3,920 runs
# (0.153%) — and those six are NOT false positives, they are the real 00:58Z-02:24Z
# `pr-gate` queue event of 2026-07-28, which is exactly what this mode exists to report.
...
def _lane(workflow="a.yml", crons=("*/10 * * * *",), cron_only=False, in_scope=True,
          state="active", fires=0, now=None, spacing_min=20):
...
def _run_obj(status="in_progress", event="schedule", path=".github/workflows/a.yml",
    ...
    return {"id": 1, "name": name, "path": path, "event": event, "status": status,
            "created_at": stamp, "run_started_at": stamp, "head_branch": "master"}
