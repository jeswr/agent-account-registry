...
def render(eligible, rows, pool, threshold, maintainer, probe_empty=False, redact_handles=False,
    ...
    if redact_handles:
        ...
        return "\n".join([
            ...
            "`ALERT_REPO` together with an `ALERT_TOKEN` secret. The private route is used only "
            "when BOTH are set, `ALERT_REPO` is not this registry, and `ALERT_REPO` is confirmed "
            "**private** by a read under `ALERT_TOKEN` — a public destination keeps this alert "
            "redacted here.\n",
...
def main():
    ...
    # ALERT_TOKEN is PRESENT, ALERT_REPO differs from the registry, AND the destination is CONFIRMED
    # private by a live GET /repos read (#432 r1 / #436 — see _alert_route's docstring); otherwise
    # fall back to the registry repo (issue #39), REDACTED, so a half-configured deployment never
    # silently drops the alert and the public fallback never enumerates the fleet. #1021: presence
    # is presence — nothing here probes the token's write capability at ALERT_REPO, and this comment
...
def _test_alert_route_contract(chk):
    ...
    `GET /repos/{ALERT_REPO}` — which establishes the destination's VISIBILITY (#432 r1), not the
    token's ability to write an issue there. So a comment saying the private destination is
    ...
    the #945 mutually-masking-duplicate shape applied to prose. It matters most HERE: this is the
    router the other copies say they mirror, and the only one whose body enumerates account handles.
    ...
    load-bearing one. So guard 1 is now CLOSED — exact equality over the whole governed text:
      ...
      1. CANONICAL PROSE, pinned CLOSED (phrasing-independent, load-bearing). The router's ENTIRE
         docstring, and the ENTIRE route comment on this script's step in dispatch.yml, are pinned by
    ...
    the route are set.
    """
    # GUARD 1, second governed consumer — dispatch.yml's route comment, which no module scan can
    # reach. Copied here VERBATIM (leading `#` and indentation stripped) and pinned by EQUALITY.
    canonical_workflow_comment = """
    Privacy routing (locked decision 22c, issue #107): the per-account alert body enumerates
    account handles, so it is emitted ONLY over a VERIFIED private route — a maintainer-set
    ALERT_REPO, an ALERT_TOKEN PRESENT beside it, ALERT_REPO distinct from this registry,
    AND that destination CONFIRMED private by a live GET /repos read under ALERT_TOKEN
    (#432 round 1 / #436). Any fallback to this PUBLIC registry repo (route unset, half-
    configured, same-repo, or unverifiable) redacts handles to a generic degraded signal.
    #1021: the verification is of the destination's VISIBILITY. Nothing probes the token's
    write capability there, so this comment must not say that it does.
    """
    ...
    anchor = "AND the destination CONFIRMED private by a live GET /repos/{ALERT_REPO} under"
    ...
    # The WORKFLOW consumer. dispatch.yml's route comment restates this contract to whoever reads
    ...
    # not on the file: a renamed or deleted dispatch.yml with the directory present raises out of the
    ...
    def workflow_pin_verdict(tree_present):
        ...
        if not tree_present:
            return sparse_1021
        with open(os.path.join(workflows_1021, "dispatch.yml"), encoding="utf-8") as handle:
            return route_comment(handle.read(), "scripts/usage-alert.py")
    ...
    chk("#1021: dispatch.yml's route comment on the step that runs THIS script is EXACTLY the "
    ...
    chk("#1021: PRESENCE + CONFIRMED-private, not capability — a credential that obviously "
        "cannot write anything still selects ALERT_REPO unredacted, exactly as a good one does",
        _alert_route("org/priv-1021", "not-a-credential-1021", "org/reg-1021",
                     confirmed_private=lambda r, t: True),
        ("org/priv-1021", "not-a-credential-1021", False))
    probe_1021 = []
    chk("#1021: ... and an EMPTY ALERT_TOKEN never does — registry, REDACTED, visibility check "
        "left uncalled",
        (_alert_route("org/priv-1021", "", "org/reg-1021",
                      confirmed_private=lambda r, t: probe_1021.append(r) or True),
         probe_1021),
        (("org/reg-1021", None, True), []))
