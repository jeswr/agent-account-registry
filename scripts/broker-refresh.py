...
def _self_test():
    ...
    # .signature, unverified payload) because the refresh decision reads its `exp` claim. This block
    # only ever needs an `exp` claim, so it wraps the `_jwt(claims)` helper above instead of
    # shadowing it — the #574 checks pass full claim dicts through that same name.
    def _exp_jwt(exp):
        return _jwt({"exp": exp})

    now = 1_800_000_000
    fresh = {"OPENAI_API_KEY": None, "auth_mode": "chatgpt",
             "tokens": {"id_token": "ID_v1", "access_token": _exp_jwt(now + 10 * 24 * 3600),
    ...
    stale["tokens"]["access_token"] = _exp_jwt(now - 3600)   # already expired
    ...
    chk("access token inside the leeway window needs a refresh",
        needs_refresh("openai", {"tokens": dict(fresh["tokens"],
                                                access_token=_exp_jwt(now + 60))}, now) is True)
    ...
    def flaky(url, payload):
        ...
        return 200, json.dumps({"access_token": _exp_jwt(now + 864000), "refresh_token": "REFRESH_v2",
                                "id_token": "ID_v2"})
    ...
    def rotating(url, payload):
        return 200, json.dumps({"access_token": _exp_jwt(now + 864000),
    ...
    crossed = False
    try:
        merge_refreshed("openai", stale, {"access_token": _exp_jwt(now + 10),
