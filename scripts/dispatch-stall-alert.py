...
def _alert_route(alert_repo, alert_token, registry_repo):
    ...
    ONLY when ALERT_TOKEN is PRESENT (a non-empty string); a half-configured deployment (repo set,
    ...
    the code has never enforced. The dispatch-stall bodies are handle-free (run links, counts,
    timestamps), so the weaker contract is not a disclosure; it must still be stated honestly,
