...
def _test_alert_route_contract(chk):
    ...
    Privacy routing (locked decision 22c): scripts/plan-alert.py routes to a maintainer-set
    PRIVATE ALERT_REPO only when ALERT_TOKEN is PRESENT, ALERT_REPO differs from this
    registry, and that destination is CONFIRMED private by a live GET /repos read
    (usage-alert.py's audited _alert_route semantics, issue #39, hardened by #432 r1/#436);
    otherwise the registry repo. The body carries no account handles (a PLAN failure is
    target-planner-side), so that fallback is safe.
    ...
    def workflow_pin_verdict(tree_present):
        ...
        with open(os.path.join(workflows_1021, "dispatch.yml"), encoding="utf-8") as handle:
            return route_comment(handle.read(), "scripts/plan-alert.py")
