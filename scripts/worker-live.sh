...
self_test() {
  ...
  # [#910; #889 found the same defect independently, from the workflow side] NOT
  # `grep -n … | head -1 | grep -c …`. `head` exits after its one line, the still-running
  ...
  # commit before #888 added the scanner, so nothing in #900's own tree could ever see it. #889 hit
  # it from the other end: extending that scanner to workflow `run:` blocks put Guard 2 (scripts/*.sh
  # == 0 hits) red on its branch too, so both lanes independently converged on removing this shape.
  #
