...
def _artifact_name(artifact):
    """How a package HOLDER is named in a conflict-attribution line.

    [#768 x Gate A] Since #798 the readiness input carries PULL-REQUEST rows too, and
    `unit_reservations` attributes a PR-led unit to the PR — so the holder may be either kind and
    the line must say WHICH. Naming a PR holder `issue#N` would point an operator at a different,
    existing artifact that happens to share the number, which is worse than saying nothing. A
    pre-seeded `in_progress_packages` entry has no artifact behind it and says so rather than
    inventing one.
    """
    if artifact is None:
...
def compute_ready(issues, in_progress_packages=None, log=None, source_links=None,
    ...
    for areas, artifact in unit_reservations(issues, source_links):
        for pkg in areas:
            holders.setdefault(pkg, artifact)
        taken |= areas

    def held_by(pkgs):
        """The (area, holder) blocking `pkgs`, or None. Mirrors the three conditions the frontier
        loop below applies exactly: a GLOBAL reservation blocks everything; an overlapping package
        blocks; and a cross-cutting candidate cannot co-run with ANY reserved package."""
        if GLOBAL in taken:
...
def _self_test():
    ...
    # [#768 x Gate A] Since #798 the readiness input carries PULL-REQUEST rows and
    # `unit_reservations` attributes a PR-led unit to the PR, so a holder may be either kind. The
    # attribution line must name WHICH: `issue#40` and `pr#40` are different, simultaneously
    # existing artifacts, and pointing an operator at the wrong one is worse than naming neither.
    pr_holder = []
    pr_row = dict(iss(40, R + ["priority:P0", "area:usage"]), pull_request={})
    compute_ready([pr_row, iss(41, R + ["priority:P1", "area:usage"])],
                  conflict_log=pr_holder.append)
    check("a PR holder is named as a PR, never as the issue that shares its number",
          pr_holder, ["conflict #41: area usage held by pr#40"])
